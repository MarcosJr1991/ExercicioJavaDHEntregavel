            ...
            for(int i=0; i <curso.size();i++){
                if(curso.get(i).getCodigoCurso()==codigoCurso){
                    for(int j=0; j < aluno.size(); j++){
                        if(aluno.get(i).getCodigoAluno()==codigoAluno){
...
/*AS estruturas de repetição também conhecidas como laços(loops) e são utilizados para executar,repetidamente,
uma instrução ou bloco de instrução enquanto determinada condição estiver sendo satifeita.
